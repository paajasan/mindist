...
import mindist
from mindist._f2py import mindist_grid
from mindist._f2py import invert as f2py_invert
from mindist._f2py import mindist_PBC as mindist_pbc
from mindist._f2py import mindist_PBC_grid as mindist_pbc_grid
print(mindist._f2py.__file__)
m_time = os.path.getmtime(mindist._f2py.__file__)
dt_m = datetime.datetime.fromtimestamp(m_time)
print(dt_m)

from mindist import mindist_cuda_pbc, mindist_omp as mindist_fort, mindist_pbc as min_pbc, mindist
import numpy as np
...
mpbc  = mindist_pbc(a,center, box)
mbrut = mindist(a,center, use_cuda=False, bruteforce=True) 
zeros1 = mbrut-mindist_fort(a,center) 

zeros2 = mbrut-mindist(a,center, use_cuda=True, bruteforce=True)

invbox = np.linalg.inv(box)

unitpos_a, unitpos_c = f2py_invert(a,center,box)
ua = a @ invbox %1
uc = center @ invbox %1
...
print(100*np.sum(zeros2<1e-12)/len(zeros2), "%")
...
selfdiff = np.abs(mpbc-mindist_pbc(a+100,center+100, u1.trajectory[0].triclinic_dimensions))
print("pbc  selfdiffs")
...
diffs = np.abs(mbrut-mindist_pbc(a,center, u1.trajectory[0].triclinic_dimensions*10))
...
mcudapbc = mindist_cuda_pbc(a,center, u1.trajectory[0].triclinic_dimensions) 
diffs = np.abs(mcudapbc-mpbc)
print("cuda pbc  diffs")
print(np.max(diffs))
print(diffs)
print(mcudapbc)
print(mpbc)
...
mgridpbc = mindist_pbc_grid(a,center, u1.trajectory[0].triclinic_dimensions, 15) 
...
for i, (asel, csel) in enumerate(zip((a1,a2,a3),(c1,c2,c3))):
    ...
    print("    ", res)
    res = np.array(timeit.repeat(lambda: mindist(a,center, use_cuda=True, bruteforce=True), repeat=rep, number=num))/num
    print(f"cuda         ", np.min(res), end=", ")
    print("    ", res)
    res = np.array(timeit.repeat(lambda: min_pbc(a,center, box, bruteforce=True, use_cuda=True), repeat=rep, number=num))/num
    print(f"cuda PBC     ", np.min(res), end=", ")
    print("    ", res)
