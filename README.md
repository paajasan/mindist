...
This will not install numba or the cudatoolkit, so if you want to use the GPU, you can either install them manually or run
```
pip install .[cuNN]
```
where `NN` should be substituted to 12 or 13 depending on the required cuda version (e.g. `pip install .[cu12]`).
...
The module includes the functions listed below. Generally you should only need `mindist` and `mindist_pbc`.

You can use them e.g.
...
With `mdtraj` you can download the whole trajectory and apply analysis in a single step.

```python
import mindist
import mdtraj

trj = mdtraj("traj.xtc",top="struct.gro")

lipids = trj.top.select("resname POPC")
prot   = trj.top.select("protein")
box = trj.unitcell_vectors
# Calculate minimum distances from atoms in prot to any atom in lipids
mind = mindist.mindist_pbc(trj.xyz[:,prot], trj.xyz[:,lipids], box)
```

### mindist()

Calculate the minimum distance for each of the *N* atom in **a** to any of the *M* atom in **center**. Can run over single frames or trajectories of frames if **a** and **center** are 3D with the same size first dimension.
...
A grid based approach, scales roughly as $\mathcal O(n_cn_a\log(n_c)+Nn_cm)$, where $n_c$ and $n_a$ are the number of gridpoints for just the center, and both groups, respectively. $m$ is the average number of (center group) atoms in the gridcells, excluding empty cells. Runs on a single frame only.

#### mindist_omp()

A brute force approach, accelerated only with OpenMP. Scales as $\mathcal O(NM)$. Runs on a single frame only.

#### mindist_omp_traj()

Same as `mindist_omp`, but runs over trajectories. OpenMP acceleration is applied over the trajectory only.
...
Runs on a single frame only.

#### mindist_omp_traj()

Same as `mindist_cuda`, but runs over trajectories.

### mindist_pbc()

Like `mindist()`, but takes the periodic boundary conditions into account. Assumes they are applied in all three directions. Works by doing the calculations in reciprocal space (relative to the box vectors). Should work for any box defined by a 3 by 3 matrix of box vectors (e.g. for cubic box a diagonal matrix with box lengths on the diagonal). Can run over single frames or trajectories of frames if **a**, **center** and **box** are 3D with the same size first dimension.

#### mindist_pbc_grid()

Like `mindist_grid()`, but takes the periodic boundary conditions into account. Runs on a single frame only.

#### mindist_pbc_omp()

Like `mindist_omp()`, but takes the periodic boundary conditions into account. Runs on a single frame only.

#### mindist_pbc_cuda()

Like `mindist_cuda()`, but takes the periodic boundary conditions into account. Runs on a single frame only.